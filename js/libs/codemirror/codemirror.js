...
(function(mod) {
  ...
  ContentEditableInput.prototype = copyObj({
    ...
    showPrimarySelection: function() {
      ...
      try {
        var rng = range(start.node, start.offset, end.offset, end.node); } catch (e) {} // Our model of the DOM might be outdated, in which case the range we try to set can be impossible
      if (rng) {
  ...
  function runInOp(cm, f) {
    ...
    startOperation(cm);
    try {
      return f(); } finally { endOperation(cm); }
  }
  ...
  function operation(cm, f) {
    return function() {
      ...
      startOperation(cm);
      try {
        return f.apply(cm, arguments); } finally { endOperation(cm); }
    };
  }
  ...
  function methodOp(f) {
    return function() {
      ...
      startOperation(this);
      try {
        return f.apply(this, arguments); } finally { endOperation(this); }
    };
  }
  ...
  function docMethodOp(f) {
    return function() {
      ...
      startOperation(cm);
      try {
        return f.apply(this, arguments); } finally { endOperation(cm); }
    };
  }
  ...
  function registerEventHandlers(cm) {
    ...
    d.dragFunctions = {
      ...
      over: function(e) {
        if (!signalDOMEvent(cm, e)) { onDragOver(cm, e);
          e_stop(e); } },
      ...
      if (cmp(cur, lastPos) != 0) {
        ...
        if (cur.line >= visible.to || cur.line < visible.from)
          setTimeout(operation(cm, function() {
            if (counter == curCount) extend(e); }), 150);
      } else {
  ...
  function handleKeyBinding(cm, e) {
    ...
    if (e.shiftKey && !cm.state.keySeq) {
      ...
      return dispatchKey(cm, "Shift-" + name, e, function(b) {
        return doHandleBinding(cm, b, true); }) || dispatchKey(cm, name, e, function(b) {
  ...
  function onBlur(cm) {
    ...
    setTimeout(function() {
      if (!cm.state.focused) cm.display.shift = false; }, 150);
  }
  ...
  function findPosV(cm, pos, dir, unit) {
    ...
    for (;;) {
      ...
      if (dir < 0 ? y <= 0 : y >= doc.height) { target.hitSide = true;
        break; }
      y += dir * 5;
    }
    return target;
  ...
  CodeMirror.prototype = {
    ...
    findWordAt: function(pos) {
      ...
      if (line) {
        ...
        var check = isWordChar(startChar, helper) ? function(ch) {
          return isWordChar(ch, helper); } : /\s/.test(startChar) ? function(ch) {
    ...
    setSize: methodOp(function(width, height) {
      ...
      cm.doc.iter(lineNo, cm.display.viewTo, function(line) {
        if (line.widgets)
          for (var i = 0; i < line.widgets.length; i++)
            if (line.widgets[i].noHScroll) { regLineChange(cm, lineNo, "widget");
              break; }
            ++lineNo;
      });
      cm.curOp.forceUpdate = true;
  ...
  option("disableInput", false, function(cm, val) {
    if (!val) cm.display.input.reset(); }, true);
  ...
  var commands = CodeMirror.commands = {
    ...
    goLineStart: function(cm) {
      cm.extendSelectionsBy(function(range) {
        return lineStart(cm, range.head.line); }, { origin: "+move", bias: 1 });
    },
    ...
    goLineEnd: function(cm) {
      cm.extendSelectionsBy(function(range) {
        return lineEnd(cm, range.head.line); }, { origin: "+move", bias: -1 });
    },
  ...
  StringStream.prototype = {
    ...
    hideFirstChars: function(n, inner) {
      this.lineStart += n;
      try {
        return inner(); } finally { this.lineStart -= n; }
    }
  };
  ...
  function removeClearedSpans(spans) {
    ...
    for (var i = 0, out; i < spans.length; ++i) {
      if (spans[i].marker.explicitlyCleared) {
        if (!out) out = spans.slice(0, i); } else if (out) out.push(spans[i]);
    }
  ...
  var off = CodeMirror.off = function(emitter, type, f) {
    if (emitter.removeEventListener)
      ...
      for (var i = 0; i < handlers.length; ++i)
        if (handlers[i] == f) { handlers.splice(i, 1);
          break; }
    }
  };
  ...
  if (document.createRange) range = function(node, start, end, endNode) {
    ...
    try { r.moveToElementText(node.parentNode); } catch (e) {
      return r; }
    r.collapse(true);
