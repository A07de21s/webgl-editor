...
(function(root, mod) {
  ...
  AVal.prototype = extend(ANull, {
    ...
    toString: function(maxDepth, parent) {
      ...
      return simplified.map(function(tp) {
        return toString(tp, maxDepth, parent); }).join("|");
    },
    ...
    makeupType: function() {
      ...
      for (var i = this.forward.length - 1; i >= 0; --i) {
        ...
        if (hint && !hint.isEmpty()) { guessing = true;
          return hint; }
      }
  ...
  var Prim = exports.Prim = function(proto, name) { this.name = name;
    this.proto = proto; };
  ...
  Obj.prototype = extend(Type.prototype, {
    ...
    unregPropHandler: function(handler) {
      for (var i = 0; i < this.onNewProp.length; ++i)
        if (this.onNewProp[i] == handler) { this.onNewProp.splice(i, 1);
          break; }
      this.maybeUnregProtoPropHandler();
    },
    gatherProperties: function(f, depth) {
  ...
  exports.withTimeout = function(ms, f) {
    ...
    timeout = end;
    try {
      return f(); } finally { timeout = oldEnd; }
  };
  ...
  var parse = exports.parse = function(text, passes, options) {
    var ast;
    try { ast = acorn.parse(text, options); } catch (e) { ast = acorn_loose.parse_dammit(text, options); }
  ...
  function makePredicate(origins, start, end) {
    ...
    if (arr && origins.length == 1) { origins = origins[0];
      arr = false; }
    if (arr) {
