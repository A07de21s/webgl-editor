Sidebar.Geometry.SphereGeometry = function(editor, object) 
{
  ...
  var widthSegments = 
    new UI.Integer(parameters.widthSegments).setRange(1, Infinity).onChange(update);

  widthSegmentsRow.add(new UI.Text('宽度分段数').setWidth('90px'));
  ...
  var heightSegments = 
    new UI.Integer(parameters.heightSegments).setRange(1, Infinity).onChange(update);

  heightSegmentsRow.add(new UI.Text('高度分段数').setWidth('90px'));
  ...
  phiStartRow.add(new UI.Text('φ start').setWidth('90px'));
  ...
  phiLengthRow.add(new UI.Text('φ length').setWidth('90px'));
